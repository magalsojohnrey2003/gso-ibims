    ...
    <header class="bg-white shadow-sm">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                    <img src="{{ asset('images/logo2.png') }}" alt="GSO-IBIMS Logo" class="h-12 w-auto object-contain"><div>
                    <p class="text-lg font-semibold">GSO-IBIMS</p>
                    <p class="text-xs text-gray-500 uppercase tracking-wide">Borrow Items Preview</p>
                </div>
            </div>
            <a href="{{ route('login') }}" class="px-5 py-2 rounded-full bg-purple-600 text-white font-semibold hover:bg-purple-700 transition">
                Log in / Register
            </a>
        </div>
    </header>
        ...
        <section class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            @forelse($items as $item)
                <article class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col">
                    <img src="{{ $item->photo_url }}" alt="{{ $item->name }}" class="h-40 w-full object-cover rounded-xl mb-4">
                    <div class="flex-1 space-y-1">
                        ...
                        @if(filled($item->description))
                            <p class="text-sm text-gray-600 mt-3 line-clamp-3">{{ $item->description }}</p>
                        @endif
                    </div>
                    <a href="{{ route('login') }}"
